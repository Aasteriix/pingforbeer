...
export default function Dashboard() {
  ...
  const pendingCount = useMemo(() => {
    ...
    for (const p of items) {
      ...
      if (
        mine &&
        (mine.status === "pending" || mine.status === "maybe")
      )
        c++;
    }
    return c;
  ...
  const nextPing = useMemo(() => {
    const future = items.filter(
      (p) => new Date(p.starts_at) > new Date(),
    );
  ...
  const canSend =
    title.trim() && location.trim() && startsAt && !sending;
        ...
        <section className="cards section">
          ...
          <div className="card">
            ...
            <div className="card-value">
              {nextPing
                ? new Date(nextPing.starts_at).toLocaleString()
                : "—"}
            </div>
            <div className="card-sub">
              {nextPing
                ? `${nextPing.activity_label || "Vibe"} • ${
                    nextPing.title
                  }`
                : "No upcoming vibes"}
            </div>
        ...
        <section className="list section">
          ...
          {items.length === 0 && (
            <div className="kicker">
              No vibes yet. Set the first one ✨
            </div>
          )}
          {items.map((p) => (
          ...
          >
            ...
            >
              ...
              <div className="inputs">
                ...
                />
                <div className="kicker">
                  Invites: all connections ({myFriends.length}).
                </div>
                {err && (
                  <div style={{ color: "#ff6a6a" }}>{err}</div>
                )}
